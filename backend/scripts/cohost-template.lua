--[[
    ...
    Version: 25.3.1 (First-to-Heartbeat Logic)
    
    This script is a template used by the backend to generate a unique script for co-hosts.
]]

return function()
    --// Dynamic Configuration - These values are injected by the backend \\--
    local authToken = "%%TEMP_AUTH_TOKEN%%" -- This starts as a temporary token
    local serverId = "%%CONTRACT_ID%%" -- The contract ID serves as the serverId for tasks
    ...
    local function sendHeartbeat()
        ...
        if success and response and response.StatusCode == 200 then
            local success, responseBody = pcall(function() return HttpService:JSONDecode(response.Body) end)
            if not success then isRunning = false; return end

            -- [MODIFIED] Logic to handle the token switch after the first heartbeat.
            if responseBody.newAuthToken then
                print("Co-Host Bot: Contract secured. Switching to permanent auth token.")
                authToken = responseBody.newAuthToken
            end
