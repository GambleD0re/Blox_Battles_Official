...
return function(config)
    ...
    pcall(function() if syn and syn.request then getgenv().syn.request = syn.request; print("Bot: Synapse request function override enabled.") end end)
    ...
    local function sendHeartbeat()
        ...
        if config.mode == "cohost" then headers["X-Cohost-Token"] = authToken else headers["X-API-Key"] = authToken end

        local req = { Url = HEARTBEAT_URL, Method = "POST", Headers = headers, Body = payload }
        
        local success, response = sendRequest(req)
        if success and response and response.StatusCode == 200 then
            print("Bot: Heartbeat sent successfully.")
            if config.mode == "cohost" then
                local successBody, responseBody = pcall(function() return HttpService:JSONDecode(response.Body) end)
                if not successBody then print("Bot: Heartbeat response was invalid JSON. Stopping."); isRunning = false; return end
                if responseBody.newAuthToken then print("Bot: SUCCESS! Contract secured. Switching to permanent auth token."); authToken = responseBody.newAuthToken end
                if responseBody.command == "shutdown" then print("Bot: Shutdown command received from backend."); isRunning = false end
            end
        else
            warn("Bot: Heartbeat failed. Stopping script. Status: " .. tostring(response and response.StatusCode))
            isRunning = false
        end
    end
            ...
            if successBody and typeof(tasks) == "table" then
                for _, task in ipairs(tasks) do
                    if task.task_type == "REFEREE_DUEL" and task.payload and task.payload.websiteDuelId then
                        print("Bot: Received task for websiteDuelId:", task.payload.websiteDuelId)
                        activeTasks[task.payload.websiteDuelId] = task
                    end
                end
            end
        else
            warn("Bot: Failed to fetch tasks from backend for " .. serverId)
        end
    end
    ...
    local function markTaskComplete(taskId)
        ...
        sendRequest(req)
        print("Bot: Marked task", taskId, "as complete.")
    end
    ...
    local function main()
        ...
        print("Bot: Waiting for game modules...")
        repeat
            pcall(function()
        ...
        if not (DuelController and SpectateController) then
            warn("Bot: FAILED to find required game modules after " .. MODULE_WAIT_TIMEOUT .. " seconds. The script will not run.")
            return
        end
        
        print("Bot: SUCCESS! Modules loaded. Initializing for serverId/contractId: " .. serverId)
        ...
        print("Bot: SUCCESS! Main loops started. Sending initial heartbeat...")
        sendHeartbeat()

        while isRunning do task.wait(1) end

        print("Bot: Initiating graceful shutdown...")
        sendBatch(); sendHeartbeat()
